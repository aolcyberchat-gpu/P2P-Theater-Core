...
const CFG = {
  ...
  // Instances that block CORS on their MP4 fileUrls.
  // HLS-only instances are no longer blocked — hls.js handles them now.
  // Only list instances where even HLS streams fail cross-origin.
  corsBlocklist: [
    // Add instances here as discovered to be completely unplayable cross-origin
  ],

  idbName:    'HiveStream4',
  idbVersion: 3,  // bumped to clear stale torrent data from v2 (ArrayBuffer corruption)
};
...
const S = {
  ...
  renderToTimer: null,
  hlsPlayer: null,   // active hls.js instance
...
function startSwarmThenRelay(hash, ws, dn) {
  ...
  if (ws) {
    ...
    idbGetTorrent(hash, function (cachedBuf) {
      ...
      let t;
      try {
        t = cachedBuf ? S.wt.add(cachedBuf, { announce: CFG.trackers }) : S.wt.add(magnet);
      } catch (e) {
        t = S.wt.get(hash);
  ...
  idbGetTorrent(hash, function (cachedBuf) {
    ...
    try {
      if (cachedBuf) {
        ...
        t = S.wt.add(cachedBuf, { announce: CFG.trackers });
      } else {
        t = S.wt.add(magnet);
      }
    } catch (e) {
...
function attachVideo(t, wsUrl) {
  ...
  // Mobile browsers (Android Chrome) cap Blob URLs at ~200MB.
  // Desktop browsers have no meaningful limit — allow renderTo for large files on desktop.
  const isMobile = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
  const BLOB_LIMIT = isMobile ? 200 * 1024 * 1024 : 4 * 1024 * 1024 * 1024; // 200MB mobile, 4GB desktop
  ...
  // Codec check: warn if file is likely HEVC/x265 (not supported in Firefox)
  // or a container the browser can't handle via MediaSource
  const lname = best.name.toLowerCase();
  const isHevc = lname.indexOf('x265') >= 0 || lname.indexOf('hevc') >= 0 || lname.indexOf('h265') >= 0;
  const isMkv = lname.endsWith('.mkv') || lname.endsWith('.mk3d');
  const mseSupported = (function() {
    if (!window.MediaSource) return false;
    if (isMkv) {
      // MKV: check if browser supports VP9 or H.264 in Matroska container
      const vp9ok = MediaSource.isTypeSupported('video/webm; codecs="vp9"');
      const h264ok = MediaSource.isTypeSupported('video/mp4; codecs="avc1.42E01E"');
      return vp9ok || h264ok; // MKV MAY work if codec is compatible
    }
    if (lname.endsWith('.mp4') || lname.endsWith('.m4v')) return true; // fMP4 assumed
    if (lname.endsWith('.webm')) return MediaSource.isTypeSupported('video/webm');
    return true; // unknown — try anyway
  })();
  if (isHevc) {
    const hevcOk = window.MediaSource && MediaSource.isTypeSupported('video/mp4; codecs="hvc1"');
    if (!hevcOk) {
      warn('HEVC/x265 not supported in this browser. File:', best.name);
      chat('⚠ "' + best.name + '" is x265/HEVC encoded — not supported in Firefox. Try Chrome or a different video.');
      if (wsUrl) httpFallback(wsUrl);
      else { setOverlay('HEVC/x265 not supported. Firefox cannot play this file.'); S.videoAttached = false; }
      return;
    }
  }

  log('P2P renderTo attempt:', best.name, '(' + Math.round(best.length / 1e6) + 'MB)', isMkv ? '[MKV]' : '[MP4/WebM]');
  ...
  if (S.hlsPlayer) { S.hlsPlayer.destroy(); S.hlsPlayer = null; }
  S.playbackSource = 'P2P';
  };
...
function playDirect(url, fallbacks) {
  // Route HLS m3u8 streams through hls.js
  if (/\.m3u8/i.test(url) || url.indexOf('/master.') >= 0 || url.indexOf('/playlist') >= 0) {
    log('detected HLS stream — using hls.js');
    playHLS(url, fallbacks);
    return;
  }
...
// HLS playback via hls.js — handles .m3u8 playlists from PeerTube HLS-only instances.
// hls.js fetches segments, decodes them, pipes into MediaSource API.
// Falls back to native HLS (Safari) if hls.js not available.
function playHLS(m3u8Url, fallbacks) {
  log('playHLS:', m3u8Url.slice(0, 90));
  try { S.playbackSource = 'HLS:' + new URL(m3u8Url).hostname.replace('www.',''); }
  catch(e) { S.playbackSource = 'HLS'; }

  // Safari/WebKit natively supports HLS — skip hls.js
  if (S.video.canPlayType('application/vnd.apple.mpegurl')) {
    log('native HLS (Safari)');
    setOverlay('');  // clear Connecting... overlay
    wireVideoEvents(m3u8Url, fallbacks || []);
    S.video.src = m3u8Url;
    S.video.load();
    const p = S.video.play();
    if (p && typeof p.then === 'function') {
      p.then(function () { log('native HLS play started'); })
       .catch(function (e) { log('native HLS blocked:', e.message); showTapOverlay(); });
    }
    return;
  }

  // Use hls.js for all other browsers
  if (typeof Hls === 'undefined' || !Hls.isSupported()) {
    chat('⚠ HLS not supported in this browser. Try a different video.');
    log('hls.js unavailable or not supported');
    transition('IDLE'); setOverlay('HLS not supported — try another video');
    return;
  }
  ...
  hls.on(Hls.Events.MANIFEST_PARSED, function (event, data) {
    ...
    const p = S.video.play();
    if (p && typeof p.then === 'function') {
      p.then(function () { log('HLS play started'); })
       .catch(function (e) { log('HLS play blocked:', e.message); showTapOverlay(); });
    }
  });

  hls.on(Hls.Events.ERROR, function (event, data) {
    if (!data.fatal) return; // non-fatal errors are handled internally
    errlog('HLS fatal error:', data.type, data.details);
    if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
      // Try to recover
      log('HLS network error — attempting recovery');
      hls.startLoad();
    } else if (data.type === Hls.ErrorTypes.MEDIA_ERROR) {
      log('HLS media error — attempting recovery');
      hls.recoverMediaError();
    } else {
      // Unrecoverable
      hls.destroy(); S.hlsPlayer = null;
      chat('⚠ HLS stream failed. Try another video.');
      transition('IDLE'); setOverlay('HLS stream failed');
    }
  });

  // Wire onplaying separately (hls.js sets src, not us)
  S.video.onplaying = function () {
    log('HLS video playing readyState=' + S.video.readyState);
    if (S.state === 'BUFFERING' || S.state === 'METADATA') transition('PLAYING');
    hideTapOverlay();
  };
...
// Extract the best HLS playlist URL from a PeerTube video API response
function getHlsUrl(data) {
  // Primary: streamingPlaylists array (PeerTube standard)
  if (data.streamingPlaylists && data.streamingPlaylists.length) {
    const hls = data.streamingPlaylists.find(function(p) {
      return p.type === 1; // type 1 = HLS
    }) || data.streamingPlaylists[0];
    if (hls && hls.playlistUrl) return hls.playlistUrl;
    // Some instances use files array within the playlist
    if (hls && hls.files && hls.files.length) {
      // Find 720p or best available
      const sorted = hls.files.slice().sort(function(a, b) {
      ...
      if (target && target.fileUrl) return target.fileUrl;
    }
  }
  return null;
}
...
function cmdPick(n) {
  ...
  fetch(host + '/api/v1/videos/' + uuid)
    ...
    .then(function (data) {
      ...
      if (!files.length) {
        // No WebTorrent MP4 files — check for HLS streaming playlist
        // PeerTube HLS playlists are in streamingPlaylists[0].playlistUrl or
        // can be constructed from the instance's /static/streaming-playlists/ path
        const hlsUrl = getHlsUrl(data);
        if (hlsUrl) {
          log('no WebTorrent files — using HLS playlist:', hlsUrl.slice(0, 80));
          const res = '(HLS)';
          chat('Playing: "' + data.name + '" [HLS] — via hls.js');
          setOverlay('Connecting…');
          transition('IDLE');
          // HLS has no torrent hash — play directly without WebTorrent swarm
          S.activeWs = hlsUrl;
          transition('BUFFERING');
          setOverlay('');  // clear "Connecting..." — HLS handles its own loading state
          const el = document.getElementById('currenttitle');
        ...
        transition('IDLE');
        setOverlay('Nothing Playing');
        chat('\u26a0 "' + data.name + '" has no playable files. Try another.');
        return;
      }
...
// Seed any direct MP4/WebM URL as a WebTorrent — !hs url <http-url>
// Downloads the file, creates a torrent from it, announces to WSS trackers.
// Other room users receive pieces via WebRTC — true P2P from a plain URL.
// Works for files under ~500MB (browser memory limit).
function cmdUrl(url) {
  if (S.state !== 'IDLE') { chat('Already ' + S.state.toLowerCase() + '. Use !hs stop first.'); return; }
  if (!isCoordinator()) { chat('Only the coordinator can start videos.'); return; }
  if (!url.startsWith('http')) { chat('Usage: !hs url <http-url>'); return; }

  chat('Downloading "' + url.slice(url.lastIndexOf('/')+1) + '" to seed…');
  transition('FETCHING');
  setOverlay('Downloading to seed…');

  fetch(url)
    .then(function(r) {
      if (!r.ok) throw new Error('HTTP ' + r.status);
      const size = parseInt(r.headers.get('content-length') || '0');
      if (size > 600 * 1024 * 1024) throw new Error('File too large (' + Math.round(size/1e6) + 'MB) — max 600MB for browser seeding');
      return r.arrayBuffer();
    })
    .then(function(buf) {
      transition('IDLE');
      const name = url.slice(url.lastIndexOf('/')+1).split('?')[0] || 'video.mp4';
      log('downloaded', Math.round(buf.byteLength/1e6) + 'MB — seeding as', name);
      setOverlay('Creating torrent…');
      if (!S.wt) S.wt = mkClient();

      // wt.seed() creates a real torrent from the buffer
      const file = new File([buf], name, { type: 'video/mp4' });
      S.wt.seed(file, { announce: CFG.trackers }, function(torrent) {
        const hash = torrent.infoHash;
        const ws = url; // original URL becomes the webseed
        log('seeding:', name, 'hash:', hash);
        S.torrent = torrent;
        S.activeWs = ws;
        transition('BUFFERING');

        // Relay to room — other users get both the hash AND the webseed URL
        // so they can stream via HTTP while P2P pieces accumulate
        relay(encodeRelay(hash, ws));
        chat('Seeding "' + name + '" — infoHash: ' + hash.slice(0,8) + '… relay sent');

        // Play locally
        const el = document.getElementById('currenttitle');
        if (el) el.textContent = name;
        attachVideo(torrent, ws);
        startHUD(torrent);
        startSyncTimer();

        torrent.on('wire', function(w) { log('peer connected via WebRTC:', w.remoteAddress || 'webrtc', 'total:', torrent.numPeers); });
        torrent.on('done', function() { idbStoreTorrent(torrent); chat('100% seeded — sharing with room'); });
      });
    })
    .catch(function(e) {
      transition('IDLE');
      setOverlay('Nothing Playing');
      chat('⚠ URL seed failed: ' + e.message);
    });
}
...
// Play any direct HTTP URL — !hs url <mp4-or-m3u8-url>
// For MP4/WebM: plays immediately, then seeds as WebTorrent if under 200MB
// For .m3u8: routes through hls.js
// Relay uses base64-encoded URL so all room clients play the same source
function cmdUrl(url) {
  if (!url.startsWith('http')) { chat('URL must start with http:// or https://'); return; }
  const name = url.split('/').pop().split('?')[0] || 'video';
  const isHls = /\.m3u8/i.test(url) || url.indexOf('playlist') >= 0;

  chat('Playing: "' + name + '" [URL]');
  setOverlay('Connecting…');

  if (isHls) {
    // HLS stream — play directly, no WebTorrent
    transition('BUFFERING');
    setOverlay('');
    S.activeWs = url;
    const el = document.getElementById('currenttitle');
    if (el) el.textContent = name;
    // Relay to room using a special _u command (URL direct play)
    relay('_u ' + btoa(url).replace(/=/g,''));
    playHLS(url, []);
    startSyncTimer();
    return;
  }

  // MP4/WebM — play directly via HTTP, relay URL to room
  transition('BUFFERING');
  setOverlay('');
  S.activeWs = url;
  const el = document.getElementById('currenttitle');
  ...
  // Attempt background WebTorrent seeding for small files (< 100MB)
  // Fetch as Blob → seed → get infohash → announce on WSS trackers
  // Other users with the hash can then download pieces via WebRTC
  // Note: only practical for files < ~100MB due to fetch time
}
  ...
  fetch(origin + '/api/v1/videos/' + vid)
    ...
    .then(function (data) {
      ...
      if (!files.length) {
        ...
        chat('⚠ "' + data.name + '" has no playable streams on this instance. Try another.');
        return;
      }
      const sorted = files.slice().sort(function (a, b) {
        return parseInt((b.resolution && (b.resolution.id || b.resolution.label)) || 0) -
...
function showGrid(items) {
  ...
  items.forEach(function (r, i) {
    ...
    dur.textContent = fmtDur(r.duration) + (host ? ' · ' + host : '');
...
function idbStoreTorrent(t) {
  ...
  function tryStore(attempts) {
    ...
    try {
      // Store as Blob — IDB structured clone handles Blob perfectly,
      // no Buffer/ArrayBuffer alignment issues. On load we use FileReader.
      const b = t.torrentFile;
      const blob = new Blob([b], { type: 'application/x-bittorrent' });
      if (blob.size < 20) { warn('IDB: blob too small, skipping'); return; }
      const tx = S.idb.transaction('torrents', 'readwrite');
      tx.objectStore('torrents').put(blob, t.infoHash);
      log('IDB stored:', t.infoHash, '(' + blob.size + 'B as Blob)');
...
function idbGetTorrent(hash, cb) {
  ...
  try {
    ...
    req.onsuccess = function () {
      ...
      // Result may be Blob (new format) or ArrayBuffer/Uint8Array (old format)
      if (result instanceof Blob) {
        result.arrayBuffer().then(function(buf) { cb(buf); }).catch(function() { cb(null); });
      } else if (result instanceof ArrayBuffer) {
        cb(result);
      } else if (result && result.buffer) {
        // Uint8Array or Buffer — extract ArrayBuffer
        cb(result.buffer.slice ? result.buffer.slice(result.byteOffset, result.byteOffset + result.byteLength) : result.buffer);
      } else {
        cb(null);
      }
    };
    ...
    req.onsuccess = function () {
      ...
      hashes.forEach(function (hash, idx) {
        idbGetTorrent(hash, function (buf) {
          ...
          try {
            const t = S.wt.add(buf, { announce: CFG.trackers });
...
function onCmd(sender, cmd, args) {
  ...
  // Fuzzy fix for common typos
  if (cmd === 'magent') { cmd = 'magnet'; }
  if (cmd === 'searh' || cmd === 'seach') { cmd = 'search'; }
  if (cmd === '_i') {
  ...
  if (cmd === 'url') {
    if (sender === S.myName) {
      const u = args[0];
      if (!u) { chat('Usage: !hs url <http-url-to-mp4>'); return; }
      cmdUrl(u);
    }
    return;
  }
  ...
  if (cmd === 'url') {
    if (sender === S.myName) {
      const rawUrl = args[0];
      if (!rawUrl) { chat('Usage: !hs url <direct-mp4-or-m3u8-url>'); return; }
      if (S.state !== 'IDLE') { chat('Already ' + S.state.toLowerCase() + '. Use !hs stop first.'); return; }
      if (!isCoordinator()) { chat('Only the coordinator can start videos.'); return; }
      cmdUrl(rawUrl);
    }
...
function cmdHelp() {
  chat('HiveStream v' + V + ': top | search <q> | pick <n> | <hash> | magnet <uri> | url <http-url> | pt <peertube-url> | sync | seek <s> | stop | status');
}
...
function boot() {
  ...
  loadScript('https://cdn.jsdelivr.net/npm/hls.js@1/dist/hls.min.js', function () {
    log('hls.js loaded:', typeof Hls !== 'undefined' ? 'v' + (Hls.version || '?') : 'FAILED');
  });
  loadScript('https://cdn.jsdelivr.net/npm/webtorrent@1/webtorrent.min.js', function () {
    ...
    // Usage: HS.status(), HS.S.torrent, HS.S.wt.torrents, etc.
    window.HS = {
      S: S,
      V: V,
      status: dumpStatus,
      stop: function () { stopAll(true); },
      wt: function () { return S.wt; },
      torrent: function () { return S.torrent; },
      peers: function () {
        if (!S.torrent) return 'no active torrent';
        return S.torrent.wires.map(function(w) {
          return { addr: w.remoteAddress || 'webrtc', type: w.type, dl: w.downloaded, ul: w.uploaded };
        });
      },
      idb: function () { return S.idb; },
    };
    log('window.HS exposed for console debugging');
  });
}
