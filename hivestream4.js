...
function isCoordinator() {
  ...
  if (!items.length) return S.myRank >= 3; // fallback if userlist not loaded
  const users = items.map(function (el) {
    const sp = el.querySelector('span:nth-child(2)');
    const name = sp ? sp.textContent.trim() : '';
    let rank = 0;
    const c = (el.className || '') + (sp ? sp.className || '' : '');
    if (c.includes('userlist_owner')) rank = 4;
    else if (c.includes('userlist_op')) rank = 3;
    else if (c.includes('userlist_leader')) rank = 2;
    else rank = 1;
    return { name, rank };
  }).filter(function (u) { return u.name; });
  if (!users.length) return S.myRank >= 3;
  users.sort(function (a, b) { return b.rank - a.rank || a.name.localeCompare(b.name); });
  return users[0].name === S.myName;
}
...
function startSwarm(hash, ws, dn) {
  ...
  idbGetTorrent(hash, function (cachedBuf) {
    ...
    try {
      ...
      // Already in client — get existing
      log('wt.add error:', e.message, '— trying wt.get');
      t = S.wt.get(hash);
      if (!t) {
        errlog('Cannot get torrent after add error:', e.message);
        transition('IDLE');
        return;
      }
    }
...
function bindTorrent(t) {
  ...
  t.on('error', function (e) {
    errlog('torrent error:', e.message || e);
    transition('IDLE');
