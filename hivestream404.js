  ...
  if (k && k.torrentUrl) {
    ...
    fetch(k.torrentUrl)
      ...
      .then(function (buf) {
        ...
        // Try multiple input formats — WebTorrent browser bundle behavior varies
        // ArrayBuffer is the most reliable in browser context
        let t = null;
        const tryFormats = [
          function() { return S.wt.add(buf); },              // raw ArrayBuffer
          function() { return S.wt.add(ua); },               // Uint8Array
          function() {                                        // Buffer.from if available
            if (typeof Buffer !== 'undefined') return S.wt.add(Buffer.from(ua));
            throw new Error('Buffer not available');
          },
        ];
        for (let i = 0; i < tryFormats.length; i++) {
          try {
            t = tryFormats[i]();
            log('wt.add succeeded with format', i, '(0=ArrayBuffer,1=Uint8Array,2=Buffer)');
            break;
          } catch (e) {
            log('wt.add format', i, 'failed:', e.message);
            t = null;
          }
        }
        if (!t) {
          errlog('all wt.add formats failed — using magnet fallback');
