  ...
  if (k && k.torrentUrl) {
    ...
    fetch(k.torrentUrl)
      ...
      .then(function (buf) {
        ...
        log('.torrent OK:', buf.byteLength, 'bytes, first byte: 0x' + ua[0].toString(16));
        if (!S.wt) S.wt = mkClient();

        // Check for existing torrent in wt client first
        ...
        if (existing) {
          ...
          // Relay now that we have a torrent instance
          if (ws) relay(encodeRelay(hash, ws));
          else relay('_i ' + hash + ' -');
          return;
        }

        // WebTorrent browser bundle (webpack) exposes window.Buffer after load.
        // parseTorrent inside WebTorrent requires a proper Buffer instance —
        // raw ArrayBuffer and Uint8Array are both rejected with 'Invalid torrent identifier'.
        // window.Buffer.from(Uint8Array) creates a real Buffer that parseTorrent accepts.
        let torrentInput;
        if (typeof window.Buffer !== 'undefined') {
          torrentInput = window.Buffer.from(ua);
          log('using window.Buffer input');
        } else {
          torrentInput = ua;
          log('window.Buffer unavailable — using Uint8Array (may fail)');
        }
        let t = null;
        try {
          t = S.wt.add(torrentInput, { announce: CFG.trackers });
          log('wt.add .torrent succeeded');
        } catch (e) {
          log('wt.add .torrent failed:', e.message, '— magnet fallback');
          t = S.wt.add(buildMagnet(hash, ws, k ? k.name : null));
          log('added via magnet fallback');
        }
        S.torrent = t;
        // Relay BEFORE bindTorrent so peers join while coordinator is getting metadata
        if (ws) relay(encodeRelay(hash, ws));
        else relay('_i ' + hash + ' -');
        bindTorrent(t);
      })
