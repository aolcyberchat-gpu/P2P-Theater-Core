  ...
  fetch(torrentUrl)
    ...
    .then(function(buf){
      ...
      destroyCurrent(function(){
        ...
        // Parse infoHash from torrent to check for duplicates in wt client
        // wt.get() accepts infoHash string or the buffer itself
        let existing=null;
        try{
          // Try to find by scanning wt.torrents array for matching buf
          // Safer: just try add and catch duplicate error
        }catch(e){}
        try{
          const t=S.wt.add(ua);
          S.torrent=t; bind(t);
        }catch(addErr){
          // Duplicate torrent — find it in the client by infoHash from fallback
          log('wt.add error:',addErr.message,', trying wt.get fallback');
          const hm=fallback&&fallback.match(/btih:([0-9a-f]{40})/i);
          const h=hm?hm[1].toLowerCase():null;
          const ex=h?S.wt.get(h):null;
          if(ex){log('found existing torrent:',h);S.torrent=ex;bind(ex);}
          else{errlog('cannot recover from wt.add error:',addErr.message);setSpinner(false);}
        }
      });
    })
    .catch(function(e){
...
function startMagnet(magnet){
  ...
  destroyCurrent(function(){
    ...
    // wt client may still have this hash registered from a prior add
    const existing = newHash ? S.wt.get(newHash) : null;
    if(existing){
      log('reusing existing wt torrent:',newHash);
      S.torrent=existing; bind(existing);
    } else {
      try{
        const t=S.wt.add(cleanMagnet);
        S.torrent=t; bind(t);
      }catch(e){
        errlog('wt.add magnet failed:',e.message,', recreating client');
        S.wt=mkClient();
        const t2=S.wt.add(cleanMagnet);
        S.torrent=t2; bind(t2);
      }
    }
  });
}
