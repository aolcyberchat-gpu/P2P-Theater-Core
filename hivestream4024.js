...
const KNOWN = {
  'dd8255ecdc7ca55fb0bbf81323d87062db1f6d1c': {
    ...
    // Multiple fallback URLs tried in order — different CDNs for different networks
    ws: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
    wsFallbacks: [
      'https://download.blender.org/peach/bigbuckbunny_movies/big_buck_bunny_720p_surround.mp4',
  ...
  'a88fda5954e89178c372716a6a78b8180ed4dad3': {
    ...
    wsFallbacks: [
      'https://download.blender.org/mango/movies/tears_of_steel_1080p.mov',
    ],
  },
  '6a9759bffd5c0af65319979fb7832189f4f3c35d': {
    name: 'Elephants Dream',
    torrentUrl: 'https://webtorrent.io/torrents/elephants-dream.torrent',
    ws: 'https://archive.org/download/ElephantsDream/ed_hd.avi',
    wsFallbacks: [
      'https://download.blender.org/ED/ed_1080p.avi',
...
function attachVideo(t, wsUrl) {
  ...
  // ARCHITECTURE: Try renderTo() FIRST — this is true P2P mode.
  // WebTorrent streams pieces directly into MediaSource API with zero HTTP to origin server.
  // Works for: fragmented MP4 (fMP4), WebM. PeerTube fileUrls and webtorrent.io demos are fMP4.
  // Fails for: regular MP4 (moov atom at end) — most archive.org files.
  //
  // If renderTo stalls (readyState stays at 1 for 8s) or errors, fall back to HTTP webseed.
  // HTTP fallback: coordinator streams from origin server, but WebTorrent KEEPS RUNNING
  // so other room users still receive P2P pieces from the coordinator via WebRTC.
  ...
  S.video.onerror = function () {
    ...
    S.video.load();
    if (wsUrl) {
      warn('renderTo failed — switching to HTTP fallback');
      httpFallback(wsUrl);
    } else {
  ...
  best.renderTo(S.video, function (err) {
    ...
    // If readyState stays ≤1 for 8s it is a regular (non-fragmented) MP4 — switch to HTTP
    ...
    S.renderToTimer = setTimeout(function () {
      ...
      if (S.video.readyState <= 1 && S.video.paused) {
        warn('renderTo stalled at readyState=' + S.video.readyState + ' — not fMP4, switching to HTTP');
        if (wsUrl) httpFallback(wsUrl);
        else { setOverlay('Cannot play this format. Try !hs stop.'); transition('IDLE'); }
      } else {
        log('renderTo OK — full P2P mode active readyState=' + S.video.readyState);
      }
    }, 8000);
  });
}
...
function httpFallback(wsUrl) {
  ...
  // WebTorrent keeps running — other users still get P2P pieces from us via WebRTC
  setOverlay('');
  var fallbacks = [];
