...
const CFG = {
  ...
  // ALL known WSS trackers — browser WebTorrent can only use WSS (not UDP/TCP).
  // Each tracker is an independent announce path. More paths = more peers found
  // across Verizon mobile, DSL, VPN, corporate networks etc.
  // Dead trackers are silently ignored by WebTorrent (just a warn log).
  trackers: [
    // ── Tier 1: high uptime, run by major projects ──────────────
    'wss://tracker.openwebtorrent.com',
    'wss://tracker.webtorrent.dev',
    'wss://tracker.webtorrent.io',
    // ── Tier 2: well-known public trackers ──────────────────────
    'wss://tracker.btorrent.xyz',
    'wss://tracker.fastcast.nz',
    'wss://tracker.novage.com.ua',
    'wss://tracker.opentrackr.org:1337/announce',
    'wss://tracker.files.fm:7073/announce',
    // ── Tier 3: PeerTube instance trackers ──────────────────────
    // PeerTube instances run their own WSS trackers for fediverse P2P
    ...
    // ── Tier 4: additional public WSS trackers ───────────────────
...
// Like startSwarm but relays _i to room peers only after infoHash is confirmed.
// This ensures coordinator has metadata ready before peers try to connect.
...
function bindTorrent(t) {
  ...
  t.on('ready', function () {
    ...
    // Try play again at ready in case renderTo resolved before ready
    if (S.video && S.video.src && S.video.paused) {
      const p = S.video.play();
      if (p && typeof p.then === 'function') {
        p.catch(function () { showTapOverlay(); });
      }
    }
  });
...
function attachVideo(t) {
  ...
  best.renderTo(S.video, function (err) {
    if (err) { errlog('renderTo error:', err.message); return; }
    // Try autoplay — mobile browsers may block it
    const p = S.video.play();
    if (p && typeof p.then === 'function') {
      p.then(function () {
        ...
        log('autoplay blocked (' + e.message + ') — showing tap overlay');
        showTapOverlay();
      });
    }
  });
...
function showTapOverlay() {
  // Remove existing tap overlay if any
  ...
  tap.onclick = function () {
    S.video.play().then(function () {
      hideTapOverlay();
      log('user tapped play');
    }).catch(function (e) {
      errlog('play() after tap failed:', e.message);
    });
  };
...
function cmdPick(n) {
  ...
  fetch(host + '/api/v1/videos/' + uuid)
    ...
    .then(function (data) {
      ...
      // Coordinator starts swarm, then relays AFTER infoHash confirmed
      // so peers connect to a coordinator that already has metadata
...
function cmdPickHash(hash) {
  ...
  if (k && k.torrentUrl) {
    // Pass the .torrent URL string directly to wt.add() — this is the correct
    // WebTorrent API. wt.add(url) fetches and parses internally, no Buffer needed.
    // Coordinator gets metadata immediately, then shares to peers via ut_metadata.
    transition('METADATA');
    setOverlay('Loading…');
    if (!S.wt) S.wt = mkClient();
    ...
    if (existing) {
      ...
      if (existing.files && existing.files.length) {
        transition('BUFFERING');
        attachVideo(existing); startHUD(existing); startSyncTimer();
      } else {
        bindTorrent(existing);
      }
      return;
    ...
    let t;
    try {
      // wt.add(urlString) — WebTorrent fetches .torrent internally, async
    ...
    // CRITICAL: relay to peers ONLY after infoHash fires.
    // infoHash = coordinator has parsed metadata and can serve it via ut_metadata.
    // If we relay before this, peers connect and ask for metadata but coordinator
    // doesn't have it yet — all three end up stuck waiting for each other.
    // We hook infoHash once here specifically for the relay — bindTorrent handles
    // all other events (metadata, ready, done etc).
    ...
    // Check if already has infoHash (cached torrent case)
    if (t.infoHash) {
      log('infoHash already present:', t.infoHash);
      relayOnce();
    } else {
      t.once('infoHash', function () {
        relayOnce();
      });
    }
    bindTorrent(t);
...
// Auto-seed ALL cached torrents on boot — browser is always a seeder
// while open. IDB cache = instant reseed without re-downloading.
// Coordinator also restores the last active torrent to the video player.
        ...
        idbGetTorrent(hash, function (buf) {
          ...
          try {
            ...
            t.on('metadata', function () {
              ...
              // If coordinator and this is the only/latest torrent,
              // restore it to the video player so room can resume watching
              if (isCoordinator() && idx === hashes.length - 1 && S.state === 'IDLE') {
                log('coordinator: restoring last session torrent to player');
                S.torrent = t;
                transition('SEEDING');
                attachVideo(t);
                startHUD(t);
                ...
                // Let room know they can join by sending _i relay
                // Extract webseed from KNOWN if available
...
function onCmd(sender, cmd, args) {
  ...
  if (cmd === '_i') {
    ...
    // If we already have this torrent seeding from IDB cache, just attach video
    const cached = S.wt ? S.wt.get(hash) : null;
    if (cached && cached.files && cached.files.length) {
      log('_i: already have torrent from IDB — attaching video');
      S.torrent = cached;
      transition('SEEDING');
      attachVideo(cached);
      startHUD(cached);
