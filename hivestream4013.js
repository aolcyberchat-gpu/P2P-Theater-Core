...
function bindTorrent(t) {
  ...
  t.on('ready', function () {
    ...
    // Don't call play() here — renderTo already called it.
    // The video element's onplaying/onpause handlers in attachVideo handle everything.
    log('ready event — video paused=' + (S.video ? S.video.paused : 'no el') + ' readyState=' + (S.video ? S.video.readyState : '?'));
  });
...
function attachVideo(t) {
  ...
  best.renderTo(S.video, function (err) {
    ...
    // Muted autoplay is allowed on all mobile browsers (power-save and autoplay policies).
    // We start muted, then show an unmute button rather than a tap-to-play overlay.
    // This avoids the "video-only background media paused to save power" error entirely.
    S.video.muted = true;
    const p = S.video.play();
    if (p && typeof p.then === 'function') {
      p.then(function () {
        log('muted autoplay succeeded');
        showUnmuteBtn();
      }).catch(function (e) {
        log('muted autoplay also blocked (' + e.message + ') — showing tap overlay');
        showTapOverlay();
      });
    } else {
      showUnmuteBtn();
...
function showTapOverlay() {
  ...
  tap.onclick = function () {
    // Try muted first (always succeeds), then unmute
    S.video.muted = true;
    S.video.play().then(function () {
      hideTapOverlay();
      showUnmuteBtn();
      log('tap-to-play succeeded (muted)');
    }).catch(function (e) {
...
function stopAll(announce) {
  ...
  S.videoAttached = false;  // allow next video to attach
  if (S.video) {
    S.video.onplaying = null; S.video.onpause = null; S.video.onerror = null;
    S.video.pause(); S.video.src = '';
  }
  transition('IDLE');
...
function idbStoreTorrent(t) {
  ...
  try {
    // t.torrentFile is a Buffer (Node-style). .buffer may be a shared/sliced ArrayBuffer
    // — copy just the relevant bytes to get a clean standalone ArrayBuffer.
    ...
    log('IDB stored:', t.infoHash, '(' + clean.byteLength + ' bytes)');
