...
function decodeRelayBody(body){
  if(!body||body.indexOf('<')<0)return body; // fast path: no HTML at all
  // Replace every <a href="X">...</a> with just X (the href value)
  let s=body.replace(/<a[^>]+href="([^"]*)"[^>]*>[^<]*<\/a>/gi,function(_,href){
    return href.replace(/&amp;/g,'&');
  });
  // Decode any remaining entities and strip any leftover tags
...
function onCmd(sender,rest){
  ...
  if(cmd==='_t'){
    const tUrl=args[0]||'';
    const fallback=relayArg(args,1); // rejoin in case magnet & params got split
    log('relay _t url='+tUrl.slice(0,60));
    log('relay _t fallback='+fallback.slice(0,60));
...
function startHash(hash){
  ...
  if(k){
    chat('Loading: '+k.name);
    // Use _m relay with webseed — works with zero peers via pure HTTP
    // Skip _t (.torrent fetch) since webtorrent.io and archive.org
    // serve HTML at those URLs in browser context (CORS/redirect issues)
