...
// ── UTILS ─────────────────────────────────────────────────────────
// CyTube enable_link_regex converts URLs in chat to <a href="url">url</a> in the
// socket chatMsg.msg payload. Strip that before using relay args as URLs.
function stripChatHTML(s){
  if(!s||s.indexOf('<')<0)return s; // fast path: no HTML
  // Extract href if it's a link: <a href="URL">...</a>
  const m=s.match(/href="([^"]+)"/);
  if(m)return decodeURIComponent(m[1].replace(/&amp;/g,'&'));
  // Otherwise strip all tags and decode entities
  return s.replace(/<[^>]*>/g,'').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>');
}

// When a relay arg may span multiple space-split parts (magnet URI with & params),
// rejoin and strip HTML from the whole thing.
function relayArg(parts,start,end){
  return stripChatHTML(parts.slice(start,end===undefined?undefined:end).join(' '));
}
...
function onCmd(sender,rest){
  ...
  if(cmd==='_t'){
    const tUrl=stripChatHTML(args[0]||'');
    // fallback is either a magnet (may be multi-part due to & splitting) or a hash
    // rejoin remaining args to reconstruct full magnet URI
    const fallback=relayArg(args,1);
    log('relay _t url='+tUrl.slice(0,60)+' fallback='+fallback.slice(0,60));
