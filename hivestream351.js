...
const CFG={
  ...
  topApi:'https://framatube.org/api/v1/videos?sort=-trending&count=12&hasWebtorrentVideo=true',
  wss:[
...
function pickResult(r){
  ...
  fetch(host+'/api/v1/videos/'+uuid)
    ...
    .then(function(data){
      ...
      if(!files.length&&data.streamingPlaylists)
        data.streamingPlaylists.forEach(function(pl){if(pl.files)files=files.concat(pl.files);});
      files=files.filter(function(f){return f.magnetUri||f.torrentUrl;});
      if(!files.length){chat('No torrent files for: '+(r.name||uuid));return;}
      const target=pickRes(files,CFG.defaultRes);
      chat('Playing: '+data.name);
      if(target.torrentUrl)relay('_t '+target.torrentUrl+' placeholder');
      else relay('_m '+target.magnet);
    })
    .catch(function(e){
      setSpinner(false);
...
function pickRes(files,pref){
  ...
  const norm=files.map(function(f){
    ...
    return{res:String(r),magnet:f.magnetUri||null,torrentUrl:f.torrentUrl||null};
  });
  const ex=norm.find(function(f){return parseInt(f.res)===pn;});
  if(ex)return ex;
  const s=norm.slice().sort(function(a,b){return parseInt(b.res)-parseInt(a.res);});
...
function cmdSearch(q){
  ...
  const url=CFG.searchApi+'?search='+encodeURIComponent(q)+
    '&count=12&sort=-trending&hasWebtorrentVideo=true';
  fetch(url)
...
function startTorrentUrl(torrentUrl,expectedHash){
  ...
  fetch(torrentUrl)
    ...
    .then(function(buf){
      setSpinner(false);
      log('.torrent: '+buf.byteLength+' bytes');
    ...
    .catch(function(e){
      setSpinner(false);
      warn('.torrent failed, magnet fallback: '+e.message);
      if(expectedHash&&expectedHash!=='placeholder'){
        startMagnet('magnet:?xt=urn:btih:'+expectedHash+
...
function startMagnet(magnet){
  ...
  if(!S.wt)S.wt=mkClient();
  destroyCurrent(function(){
    setSpinner(true,'Fetching metadata…');
    const t=S.wt.add(magnet);
    S.torrent=t; bind(t);
  });
}
...
function ptLoad(url,preferredRes){
  ...
  fetch(origin+'/api/v1/videos/'+vid)
    ...
    .then(function(data){
      setSpinner(false);
      let raw=data.files||[];
      if(!raw.length&&data.streamingPlaylists)
        data.streamingPlaylists.forEach(function(pl){if(pl.files)raw=raw.concat(pl.files);});
      const files=raw.filter(function(f){return f.magnetUri||f.torrentUrl;}).map(function(f){
        return{res:String((f.resolution&&(f.resolution.id||f.resolution.label))||'?'),
               magnet:f.magnetUri||null,torrentUrl:f.torrentUrl||null};
      });
      if(!files.length)throw new Error('No torrent files');
      const t=pickRes(files,preferredRes||CFG.defaultRes);
      chat('"'+data.name+'" @ '+t.res+'p');
      if(t.torrentUrl)relay('_t '+t.torrentUrl+' placeholder');
      else relay('_m '+t.magnet);
    })
