...
function idbStoreTorrent(t) {
  ...
  function tryStore(attempts) {
    ...
    try {
      // Store as Uint8Array directly — IDB supports structured clone of typed arrays.
      // .slice() creates a fully independent copy (own buffer, correct byteOffset=0).
      // This is more reliable than ArrayBuffer.slice() across browser/WebTorrent versions.
      const b = t.torrentFile;
      const u8 = (b instanceof Uint8Array) ? b : new Uint8Array(b.buffer || b, b.byteOffset || 0, b.byteLength);
      const copy = u8.slice(); // independent copy, own ArrayBuffer
      if (copy.byteLength < 20) { warn('IDB: torrent data too small, skipping'); return; }
      const tx = S.idb.transaction('torrents', 'readwrite');
      tx.objectStore('torrents').put(copy, t.infoHash);
      log('IDB stored:', t.infoHash, '(' + copy.byteLength + 'B)');
