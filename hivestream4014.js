...
function attachVideo(t) {
  ...
  // KEY RULE: renderTo() fully owns this video element.
  // It sets video.src, creates MediaSource, buffers data, and calls play() internally.
  // We must NEVER call play() ourselves — doing so races with renderTo's internal
  // call and causes "interrupted by a call to pause()" on ALL browsers.
  //
  // Wire video element events BEFORE renderTo, then just observe.
  S.video.onplaying = function () {
    log('video playing — readyState=' + S.video.readyState);
    hideTapOverlay();
    if (S.video.muted) showUnmuteBtn();
  };

  // canplay fires when browser has enough data to start.
  // If autoplay was blocked, video stays paused — show tap overlay.
  S.video.oncanplay = function () {
    log('canplay — paused=' + S.video.paused + ' readyState=' + S.video.readyState);
    if (S.video.paused) showTapOverlay();
  };

  S.video.onerror = function () {
    errlog('video error:', S.video.error ? S.video.error.message : 'unknown');
  };

  // renderTo handles src, MediaSource, buffering, and play() entirely.
  // Callback is only for error logging — do NOT call play() here.
  best.renderTo(S.video, function (err) {
    if (err) { errlog('renderTo error:', err.message); S.videoAttached = false; return; }
    log('renderTo OK — readyState=' + S.video.readyState);
  });
}
